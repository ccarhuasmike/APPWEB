...
namespace INTERSUR.INFSAP.Entidades
{
	...
	public	BEConsulta(IDataReader  oData)
	{		
	    PkId = Convert.ToDecimal(oData["PK_ID"] is DBNull ? 0 : oData["PK_ID"]);
            ...
            CNumDocA = Convert.ToString(oData["C_NUM_DOCA"] is DBNull ? "" : oData["C_NUM_DOCA"]);

            CTipDoc = Convert.ToString(oData["C_TIP_DOC"] is DBNull ? "" :oData["C_TIP_DOC"]);

            FecReg = Convert.ToString(oData["FEC_REG"] is DBNull ? "" : oData["FEC_REG"]);
            
            EstReg = Convert.ToString(oData["EST_REG"] is DBNull ? "" : oData["EST_REG"]);
